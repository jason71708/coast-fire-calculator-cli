...
describe('Utils', () => {
  ...
  describe('formatCurrency', () => {
    ...
    test('should format large amounts with proper comma separation', () => {
      ...
      expect(formatCurrency(20000000)).toBe('NT$20,000,000');
      expect(formatCurrency(100000000)).toBe('NT$100,000,000');
    });
    ...
    test('should handle decimal numbers by rounding', () => {
      expect(formatCurrency(1000.5)).toBe('NT$1,001');
      expect(formatCurrency(1000.4)).toBe('NT$1,000');
      expect(formatCurrency(1000000.99)).toBe('NT$1,000,001');
    });

    test('should handle very large numbers', () => {
      expect(formatCurrency(1000000000)).toBe('NT$1,000,000,000');
      expect(formatCurrency(999999999)).toBe('NT$999,999,999');
    });
    ...
    test('should format amounts less than 1000 without commas', () => {
      ...
      expect(formatCurrency(999)).toBe('NT$999');
      expect(formatCurrency(1)).toBe('NT$1');
    });

    test('should handle amounts with thousands separator', () => {
      expect(formatCurrency(1234567)).toBe('NT$1,234,567');
      expect(formatCurrency(987654321)).toBe('NT$987,654,321');
    });
  });
});